...
namespace AdminPanel.Controllers
{
    public class OrderController : Controller
    {
        ...
        [HttpGet]
        public IActionResult GetAll(string? status, string? orderDate, string? orderMonth, string? orderYear)
        {
            ...
            if (response != null && response.IsSuccess)
            {
                ...
                if (!string.IsNullOrEmpty(status) && string.IsNullOrEmpty(orderDate))
                {
                    switch (status)
                    {
                        case "approved":
                            ...
                            break;
                        case "readyforpickup":
                            list = list.Where(u => u.Status == SD.Status_ResdyForPickup);
                            break;
                        case "cancelled":
                ...
                else if (!string.IsNullOrEmpty(orderMonth) && !string.IsNullOrEmpty(orderYear))
                {

                    list = list.Where(x => x.OrderTime.ToString("MM") == orderMonth
                    && x.OrderTime.ToString("yyyy") == orderYear).
                    OrderByDescending(y => y.OrderHeaderId);
                }
                else if (string.IsNullOrEmpty(orderMonth) && !string.IsNullOrEmpty(orderYear))
                {
                    list = list.Where(x => x.OrderTime.ToString("yyyy") == orderYear).OrderByDescending(x => x.OrderHeaderId);
                }
                else
                    list = list.Where(x => x.OrderTime.ToString("yyyy/MM/dd") == orderDate);
            }
            else
            {
